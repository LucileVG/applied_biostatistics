...
more important among fishermen, as expected. Another surprising result
is the negative value of \(\beta_5\) (corresponding to
\emph{fisherman}:\emph{fishmlwk} interaction): it suggests that among
fishermen the number of fish meals per week has a weak impact on total
Hg levels (\(\beta_3+\beta_5 = 0.1\)) while it has a much stronger
impact on non-fishermen (\(\beta_3 = 1.53\)). However, we have to
remember that the number of fish meals per week is completely separable
by population group. The simplest explanation could be that the relation
between the number of fish meals per week and total Hg levels is
positive but not linear: it increases fast for low numbers of fish meals
(i.e.~for non-fishermen) and more slowly for high numbers of fish meals
(i.e.~for fishermen). Furthermore, the observable does not reflect
entirely the quantity of fish eaten, since one can eat more or less fish
per meal. The weight of fish eaten per week, might be a more accurate
observable to study.
