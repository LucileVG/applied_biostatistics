...
The plot of the residuals against the fitted values help us to assess 3 assumptions about the residuals. First, the mean of the residuals should be 0, and the plot confirms it. Second, the model should be homoscedastic, but the plot tends to show it is not the case. Indeed, a model is homoscedastic if the variance is the same for all the values, and here, the variance is much higher for fishermen than for non-fishermen. However, within each class, the variance is overall similar, even if it tends to be a little more spread for high fitted values. At last, for the third one, which is the uncorrelation between the X variables and the residuals, we can again confirm it with the plot.
